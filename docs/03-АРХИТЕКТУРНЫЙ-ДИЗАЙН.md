...
**Обновлено:** 24 декабря 2025 - исправлена структура проекта (main_gui.go, internal/app, frontend/), актуализирована версия Go
...
graph TB
    ...
    subgraph "Business Logic Layer"
        CONV[Converter<br/>Конвертация валют]
        VAL[Validator<br/>Валидация]
        FMT[Formatter<br/>Форматирование]
    end
    
    subgraph "Data Access Layer"
        PARSER[Parser<br/>XML API ЦБ РФ]
        CACHE[Cache<br/>LRU с TTL]
    end
    
    subgraph "External"
        CBR[CBR XML API<br/>ЦБ РФ]
    end
...
sequenceDiagram
    ...
    User->>GUI: Ввод данных<br/>(amount, currency, date)
    User->>GUI: Нажатие "Конвертировать"
    ...
    alt RUB валюта
        Converter->>Converter: Возврат без запроса к API
    else Другая валюта
        ...
        alt Cache Hit
            ...
            loop Retry (max 3 попытки)
                Parser->>CBR: HTTP GET (XML API)
                alt Success
                    CBR-->>Parser: XML (windows-1251)
                    Parser->>Parser: Конвертация UTF-8
            ...
            Converter->>Converter: Извлечение курса<br/>(с учетом номинала)
...
stateDiagram-v2
    ...
    Idle --> Validating: Пользователь вводит данные
    Validating --> Valid: Данные корректны
    Validating --> Invalid: Данные некорректны
    
    Valid --> CheckingCache: Проверка кэша
    CheckingCache --> CacheHit: Курс в кэше
    CheckingCache --> CacheMiss: Курса нет в кэше
    
    CacheMiss --> Fetching: Запрос к API
    Fetching --> Success: Успешно
    Fetching --> Error: Ошибка сети
    
    CacheHit --> Formatting: Форматирование результата
    Success --> Formatting
    Formatting --> DisplayResult: Отображение результата
    
    Invalid --> Idle: Исправление данных
    Error --> Idle: Повторная попытка
    
    DisplayResult --> Idle: Новый запрос
```
...
graph TB
    ...
    subgraph "Wails Runtime"
        BIND[Wails Bindings<br/>Автогенерация]
    end
    
    subgraph "Backend (Go)"
        APP[App Struct<br/>Wails Methods]
        CONV[Converter<br/>Существующий код]
    end
...
**Обновлено:** 24.12.2025 (v2.1 - исправлена структура проекта, добавлен frontend и internal/app)
