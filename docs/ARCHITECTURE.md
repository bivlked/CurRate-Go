...
- Кэширует результаты через Caching Layer
...
**Взаимодействие:**
- Кэширует результаты запросов к API
- Используется Data Access Layer для проверки кэша
...
5. **Converter проверяет кэш** через Cache Layer
...
9. **Parser кэширует результат** в Cache Layer
10. **Converter конвертирует валюту** используя полученные курсы
...
3. **Converter проверяет кэш** через Cache Layer
4. **Если кэш пуст**, запрос к Parser
5. **Parser получает данные** из API или кэша
6. **Converter конвертирует** и форматирует результат
...
- **[03-АРХИТЕКТУРНЫЙ-ДИЗАЙН.md](03-АРХИТЕКТУРНЫЙ-ДИЗАЙН.md)** - Детальный архитектурный дизайн
- **[12-WAILS-GUI-АРХИТЕКТУРА.md](12-WAILS-GUI-АРХИТЕКТУРА.md)** - Архитектура GUI компонента
